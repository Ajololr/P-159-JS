...
window = Tk()
window.title('Радиостанция Р-159')
...
def open_github():
    webbrowser.open('https://github.com/Ajololr/P-159-JS', new=0, autoraise=True)

def open_client():
    webbrowser.open(address, new=0, autoraise=True)


def start_server(path_to_launcher):
...
if len(server_path_arr) != 0:
    ...
    Button(window, command=lambda: start_server(server_path), width=18, text='Запуск', fg='white', bg=BACKGROUND_COLOR_BUTTON, font='Arial 10')\
